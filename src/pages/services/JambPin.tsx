...
import { BookOpen, Minus, Plus, ArrowRight, ShieldCheck, Sparkles } from 'lucide-react';
...
const JAMB_UNIT_PRICE = 5000;

export default function JambPin() {
  const navigate = useNavigate();
  const purchase = usePurchase();
  const { calculateCashback } = useCashback();
  const [quantity, setQuantity] = useState(1);
  const [unitPrice, setUnitPrice] = useState(JAMB_UNIT_PRICE);
  const [unitCashback, setUnitCashback] = useState(0);

  useEffect(() => {
    supabase.from('products').select('*').eq('service', 'jamb').eq('is_active', true).limit(1).then(({ data }) => {
      if (data && data[0]) {
  ...
  const cashbackAmount = calculateCashback(total, 'jamb', unitCashback);

  const handleProceed = () => {
    if (!canProceed) return;
    purchase.startPurchase({
      service: 'jamb', product: `JAMB UTME Pin x${quantity}`, amount: total,
      recipient: 'JAMB', network: 'JAMB', metadata: { quantity },
  ...
  return (
    <div className="min-h-screen pb-32 bg-slate-50 dark:bg-slate-950">
      <ServiceHeader title="JAMB PIN" subtitle="Buy JAMB UTME registration pins" icon={BookOpen} />

      <div className="px-5 -mt-12 relative z-10 space-y-5">
        {/* Hero card */}
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
          className="relative rounded-3xl bg-gradient-to-br from-rose-400 to-red-600 p-5 overflow-hidden shadow-xl">
          ...
          <div className="relative flex items-center gap-4">
            ...
            <div>
              <p className="text-white font-bold font-display text-lg">JAMB UTME Pin</p>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="flex items-start gap-2 px-1">
          ...
          <p className="text-[11px] text-slate-400 dark:text-slate-500 leading-relaxed">
            Your JAMB PIN(s) will be generated instantly after purchase and displayed on the receipt screen. Keep your PIN safe.
          </p>
        </motion.div>
      </div>
            ...
            <div className="card-premium p-4 shadow-2xl">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="text-right">
                  ...
                  <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">{quantity}x JAMB</p>
                </div>
              </div>
              {cashbackAmount > 0 && (
              ...
              <Button fullWidth size="lg" onClick={handleProceed}>Purchase JAMB PIN <ArrowRight className="w-4 h-4" /></Button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>

      <PurchasePinModal open={purchase.showPinModal} onClose={purchase.closePinModal} amount={total} productName={`JAMB PIN x${quantity}`}
